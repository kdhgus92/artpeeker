...
`backend/`는 .NET 10 Web API이며 앞으로도 .NET을 유지합니다.

로컬에 .NET 10 SDK가 설치되어 있어야 합니다. `dotnet --version`으로 확인하고, 설치되지 않았다면 사용하는 운영체제의 공식 설치 절차에 따라 SDK를 설치합니다.
...
- 백엔드 변경은 `dotnet build`를 통과해야 합니다.
